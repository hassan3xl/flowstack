...
const ProjectDetailPage = () => {
  ...
  const { mutateAsync: deleteItem, isPending: deletingItem } =
    useDeleteProjectItem();
  ...
  // Derived Stats
  const total = project.project_items.length;
  const completed = project.project_items.filter(
                ...
                {expandedTaskId === item.id && (
                  <div className="bg-muted/30 border-t border-border p-4 animate-in slide-in-from-top-2 duration-200">
                    ...
                    <div className="mt-6 pt-4 border-t border-border">
                      <CommentComponent
                        itemId={item.id}
                        isOpen={true} // Always open if expanded
                        serverId={serverId}
                        projectId={projectId as string}
